import { AssessmentData, FeatureVector } from "./types";
...
function questionnaire(data: AssessmentData): Record<string, number> {
...
function games(data: AssessmentData): Record<string, number> {
    const g = data.games_response ?? {};

    return {
        ...Object.fromEntries(
            Object.entries(g.laundry_sorter ?? {}).map(([k, v]) => [
                `g_laundry_sorter.${k}`,
                num(v)
            ])
        ),
        ...
        ...Object.fromEntries(
            Object.entries(g.memory_dialer ?? {}).map(([k, v]) => [
                `g_memory_dialer.${k}`,
                num(v)
            ])
        ),
        ...
        ...Object.fromEntries(
            Object.entries(g.money_manager ?? {}).map(([k, v]) => [
                `g_money_manager.${k}`,
                num(v)
            ])
        ),
        ...
        ...Object.fromEntries(
            Object.entries(g.shopping_list_recall ?? {}).map(([k, v]) => [
...
function eye(data: AssessmentData): Record<string, number> {
